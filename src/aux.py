...
def run_CER_method(swt,Nens=10):
    ...
    ensana = np.reshape(ensana,(np.shape(ssha)[0],52))

    ensanagap = ssha*np.nan
    ensanagap[:,:26] = ensana[:,:26]
    ensanagap[:,35:] = ensana[:,26:]
...
def interp_ens_to_track(ens_path,swt,Nens=10,nselect=60):
    ...
    i_swt = int( (np.array(np.mean(swt.dset.time), dtype='datetime64[D]')-np.datetime64('2012-10-01') )/ np.timedelta64(1, "D") )
